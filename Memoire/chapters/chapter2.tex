...
\textbf{Attaques par injection de code malveillant~:} Ces attaques exploitent les vulnérabilités des mécanismes de mise à jour pour injecter du code malveillant dans le firmware. L'étude de González-Manzano et al. \cite{Gonzalez2024ExploringShifting} révèle une sophistication croissante de ces attaques, avec l'adaptation de malwares initialement conçus pour d'autres plateformes.
...
\textbf{Types d'éléments sécurisés~:} Les SE peuvent être implémentés sous forme de puces dédiées, d'enclaves sécurisées dans le processeur principal, ou de modules logiciels avec protection matérielle. Chaque approche présente des avantages et des inconvénients en termes de sécurité, de performance et de coût.

\textbf{Capacités cryptographiques~:} Les SE modernes intègrent des accélérateurs cryptographiques pour les opérations de chiffrement, de signature, et de génération de nombres aléatoires. L'étude de Kim et al. \cite{Kim2024HighSecurity} présente une implémentation HSM basée sur FPGA avec des fonctions PUF (Physically Unclonable Functions) pour l'authentification.
...
\textbf{Attestation légère~:} Les protocoles d'attestation traditionnels sont souvent trop lourds pour les dispositifs IoT. Les travaux d'Abdelaziz et al. \cite{Abdelaziz2024LightweightRemote} proposent une solution basée sur des PUF et des signatures de hachage, réduisant significativement l'overhead computationnel.
...
\textbf{Architecture modulaire évolutive~:} Implémentation de la Configuration Standard basée sur cryptographie logicielle mbedTLS avec architecture permettant l'intégration progressive de mécanismes matériels (HSM, TEE) selon les besoins de sécurité, comme spécifié dans la Configuration Expert.

\textbf{Vérification d'intégrité robuste~:} Mécanismes de vérification au démarrage avec signatures cryptographiques implémentés dans la Configuration Standard, établissant les fondations pour une extension future vers la vérification temps réel continue détaillée dans la Configuration Expert.

\textbf{Détection d'anomalies adaptative~:} Système de détection configurable avec seuils adaptatifs implémenté dans la Configuration Standard, préparant l'intégration future d'algorithmes de machine learning pour une adaptation comportementale automatique comme conçu dans la Configuration Expert.

\textbf{Approche pragmatique~:} Conception orientée vers le déploiement réel sur plateforme ESP32, avec validation expérimentale utilisant des capteurs DHT22 et architecture extensible à divers types de capteurs IoT.

\textbf{Roadmap claire~:} Définition d'une feuille de route technique pour l'évolution progressive de la Configuration Standard vers la Configuration Expert avec des fonctionnalités avancées, permettant aux organisations d'adopter des niveaux de sécurité croissants selon leurs besoins.

\section{Conclusion}

Cette revue de littérature révèle que malgré les avancées significatives dans la sécurité des firmwares IoT, des lacunes importantes subsistent. Les solutions existantes peinent à offrir un équilibre satisfaisant entre sécurité, performance et praticité de déploiement. Notre contribution vise à combler ces lacunes en proposant SecureIoT-VIF, un framework qui combine efficacité de protection, overhead acceptable, et architecture évolutive. L'approche adoptée permet un déploiement avec des mécanismes de sécurité robustes dans la Configuration Standard implémentée et validée, tout en établissant les fondations pour une évolution future vers des fonctionnalités avancées spécifiées dans la Configuration Expert (vérification temps réel, HSM matériel, machine learning adaptatif).

Le chapitre suivant présente une analyse détaillée des menaces et vulnérabilités spécifiques aux firmwares IoT, fournissant les bases théoriques pour la conception et la validation de SecureIoT-VIF.
