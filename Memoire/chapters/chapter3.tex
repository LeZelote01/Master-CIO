...
\textbf{Exploitation de vulnérabilités de protocoles~:} Les dispositifs IoT utilisent divers protocoles de communication (HTTP, MQTT, CoAP, ZigBee) présentant des vulnérabilités exploitables. L'analyse de Palo Alto Networks \cite{PaloAlto2023MiraiVariant} révèle que les variants récents de Mirai exploitent des vulnérabilités comme CVE-2019-12725 (Zeroshell RCE) et CVE-2023-1389 (TP-Link Command Injection) pour compromettre les firmwares.

\textbf{Attaques par déni de service permanent~:} BrickerBot représente une catégorie particulière d'attaques visant à rendre les dispositifs IoT définitivement inutilisables. Trend Micro \cite{TrendMicro2024BrickerBot} documente comment cette famille de malwares exécute des commandes destructrices corrompant le stockage et les paramètres du noyau, causant des dommages permanents au firmware.
...
L'absence de mécanismes de fraîcheur dans les signatures de firmware permet la réplication d'anciennes signatures valides pour installer des versions vulnérables. Cette technique, appelée \textit{rollback attack}, compromet la sécurité même avec des mécanismes de signature robustes.
...
\textbf{Impact potentiel~:} Évalué sur une échelle de 1 à 5 basé sur~:
\begin{itemize}
...
Cette section établit les exigences de sécurité que SecureIoT-VIF doit satisfaire pour adresser efficacement les menaces identifiées. Les exigences sont organisées selon leur niveau de priorité et leur adéquation avec les deux configurations complémentaires de l'architecture~: la Configuration Standard (implémentée et validée) et la Configuration Expert (conçue en détail pour des déploiements nécessitant des mécanismes de sécurité avancés).

\subsection{Exigences fonctionnelles}

L'analyse des menaces identifie plusieurs exigences fonctionnelles critiques pour SecureIoT-VIF~:

\textbf{EF-1~: Vérification d'intégrité du firmware}
\begin{itemize}
    \item \textbf{Configuration Standard~:} Vérification de l'intégrité du firmware au démarrage avec signatures cryptographiques SHA-256/ECDSA
    \item \textbf{Configuration Expert~:} Extension vers la vérification continue pendant l'exécution avec détection des modifications non autorisées en moins de 100~ms
...
\begin{itemize}
    ...
    \item \textbf{Configuration Standard~:} Implémentation avec cryptographie logicielle mbedTLS
    \item \textbf{Configuration Expert~:} Extension avec HSM matériel et accélération cryptographique
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Configuration Standard~:} Architecture préparée pour l'intégration future
    \item \textbf{Configuration Expert~:} Spécifications détaillées pour implémentation complète
\end{itemize}

\textbf{EF-4~: Détection d'anomalies comportementales}
\begin{itemize}
    \item \textbf{Configuration Standard~:} Détection par seuils configurables et adaptatifs
    \item \textbf{Configuration Expert~:} Analyse avancée avec machine learning adaptatif
    \item Détection des comportements anormaux sans signatures préétablies
    \item Adaptation aux profils de comportement normaux du dispositif
\end{itemize}
...
\begin{itemize}
    \item \textbf{Configuration Standard~:} Overhead computationnel < 8~\% des ressources disponibles
    \item \textbf{Configuration Expert~:} Overhead optimisé < 5~\% avec accélération matérielle
    \item Temps de vérification < 50~ms pour des firmwares de 1--2~MB
    \item Consommation énergétique additionnelle < 2~\%
\end{itemize}

\textbf{ENF-2~: Compatibilité}
\begin{itemize}
    \item Support prioritaire de la plateforme ESP32 (Configuration Standard implémentée)
    \item Architecture extensible pour ARM Cortex-M, RISC-V, x86
    \item Intégration avec les systèmes d'exploitation embarqués populaires (FreeRTOS)
    \item Compatibilité avec les éléments sécurisés existants (Configuration Expert)
\end{itemize}

\textbf{ENF-3~: Évolutivité}
\begin{itemize}
    \item Architecture modulaire permettant l'évolution de la Configuration Standard vers la Configuration Expert
    \item Capacité d'adaptation aux nouvelles menaces
    \item Support des algorithmes cryptographiques post-quantiques (planifié dans Configuration Expert)
    \item Roadmap claire pour l'intégration progressive de fonctionnalités avancées
\end{itemize}
...
\begin{itemize}
    ...
    \item Protection contre les attaques par canaux cachés (renforcée dans Configuration Expert)
\end{itemize}
...
Cette analyse approfondie des menaces révèle la complexité et la diversité des attaques ciblant les firmwares IoT. L'évolution constante des techniques d'attaque, combinée à la prolifération des dispositifs IoT vulnérables, crée un environnement de menace particulièrement exigeant.
...
Ces conclusions guident la conception de SecureIoT-VIF, présentée dans le chapitre suivant, en définissant les exigences de sécurité prioritaires et les contraintes techniques à respecter. L'approche proposée vise à adresser les menaces identifiées tout en respectant les contraintes de performance et de compatibilité des dispositifs IoT grand public. L'architecture à deux configurations permet une adoption progressive, commençant par la Configuration Standard dont l'efficacité sera validée expérimentalement, avec une roadmap claire vers la Configuration Expert intégrant des mécanismes de sécurité avancés pour les déploiements nécessitant des niveaux de protection supérieurs.
