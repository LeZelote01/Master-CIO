...
\begin{itemize}
    ...
    \item Taux de faux positifs (FPR) acceptable pour le déploiement
...
L'évaluation de la détection d'intégrité a été menée sur 150 scénarios professionnels soigneusement conçus pour le déploiement :
...
L'analyse des faux positifs est cruciale pour un usage professionnel efficace, car des alertes erronées peuvent perturber le déploiement :
...
\subsection{Évaluation de l'efficacité pédagogique}
...
\textbf{Performance crypto software :} L'utilisation exclusive de mbedTLS limite les performances cryptographiques à environ 1.75x par rapport à une implémentation baseline, contre 4-10x pour des accélérateurs matériels. Cette limitation est acceptable pour le déploiement mais pourrait frustrer les développeurs avancés.
...
\textbf{Absence de composants externes :} L'approche tout-intégré limite le déploiement des interactions avec des composants de sécurité externes (HSM, TPM, cartes à puce).
...
\begin{enumerate}
    ...
    \item \textbf{Niveau avancé (6-8 semaines) :} Migration vers solutions Configuration Expert simulées
...
\textbf{Passerelle vers Configuration Expert :} Développement d'un module de transition permettant la découverte progressive des fonctionnalités avancées (HSM, accélérateurs, ML adaptatif) sans abandonner la plateforme professionnelle.
